...
class CVAE(nn.Module):
    """
    Conditional VAE for copper corrosion image generation.
    - encode: image features + s-parameters (s11,s21,p11,p21) -> mu/logvar
    - decode: z + corrosion_level -> reconstructed image
    - num_layers controls conv depth
    """
    def __init__(self, latent_dim: int, s_param_dim: int, num_layers: int = 3, image_size=(256, 256)):
        ...
        h, w = image_size
        if h != w:
            raise ValueError("image_size must be square (e.g. 256x256).")
