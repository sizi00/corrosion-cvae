...
class CorrosionDataset(Dataset):
    """
    Copper corrosion image dataset.
    - images: R channel only (best reflects corrosion)
    - label: corrosion rate parsed from filename
    - sparams: 4 s-parameter tables matched by corrosion rate
    """
...
def build_dataloaders(cfg: ExperimentConfig):
    # read sparam csvs
    s11 = pd.read_csv(cfg.data.sparam_paths["s11"], index_col=0)
    s21 = pd.read_csv(cfg.data.sparam_paths["s21"], index_col=0)
    p11 = pd.read_csv(cfg.data.sparam_paths["p11"], index_col=0)
    p21 = pd.read_csv(cfg.data.sparam_paths["p21"], index_col=0)
